...
public class Rule {
    ...
    private static boolean checkSoldier(int[][] cheses, int startX, int startY, int endX, int endY) {
        ...
        if (checkICamp) {
            if (startX == endX && endY - startY == result) {
                return true;
            }
        } else if (!checkICamp) {
            if (startX == endX && endY - startY == result) {
            ...
            if (startY == endY && Math.abs(startX - endX) == 1) {
                return true;
            }
        }
        return false;
    ...
    private static boolean checkGeneral(int[][] cheses, int startX, int startY, int endX, int endY) {
        int code = cheses[startX][startY];
        if (code > 0) {
            if (!(3 <= endX && endX <= 5 && 0 <= endY && endY <= 2)) {
                return false;
            }

        } else {
            if (!(3 <= endX && endX <= 5 && 7 <= endY && endY <= 9)) {
                return false;
            }
        }
    ...
    //托管棋子自动走位code 判断棋子是哪方的目前只做了兵自动补位
    public static CodeIndex goAutomic(int[][] cheses, int code) {
        CodeIndex codeIndex = null;
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 10; j++) {
                if (cheses[i][j] == CheseCode.Soldier.getCode() && code == CheseCode.Red.getCode()) {
                    if (j < 9) {
                        //前进一步
