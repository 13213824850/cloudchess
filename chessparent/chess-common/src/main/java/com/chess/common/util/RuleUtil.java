...
public class RuleUtil {
	...
	//检查棋子走位是否在棋盘 在返回true
	public static boolean checkOutBoard(CodeIndex codeIndex) {
		int startX = codeIndex.getStartX();
		if(!(startX>= 0 && startX <= 8)) {
			return false;
		}
		int startY = codeIndex.getStartY();
		if(!(startY>= 0 && startY <= 9)) {
			return false;
		}
		int endX = codeIndex.getEndX();
		if(!(endX>= 0 && endX <= 8)) {
			return false;
		}
		int endY = codeIndex.getEndY();
		return endY >= 0 && endY <= 9;
	}
